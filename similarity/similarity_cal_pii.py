...
def load_and_compute_similarity(data_path, model, tokenizer):
    """
    ...
    for item in data.values():
        ...
        forget_data.append(item['forget'])
    ...
    forget_queries = []
    for forget_list in forget_data:
        temp_f_list = []
        for item in forget_list.values():
            temp_f_list.append(item['response'])
        forget_queries.append(temp_f_list)

    # Initialize similarity matrix
    similarity_matrix = torch.zeros(len(retain_data), len(forget_queries[0]))
    ...
    for i, retain_sample in tqdm(enumerate(retain_queries), total=len(retain_queries), desc="Processing Retain Samples"):
        ...
        d_rep = l2v.encode(forget_queries[i], convert_to_tensor=True)  # shape: (embed_dim,)
        d_rep_norm = torch.nn.functional.normalize(d_rep, p=2, dim=1)
        # Compute cosine similarity with torch.dot
        similarity_matrix[i, :] = torch.mm(q_rep_norm,d_rep_norm.transpose(0,1))
...
def process_paired_files(base_path, model, tokenizer):
    # 确保输出目录存在
    output_dir = Path("/data/home/jvnting/cnpo/data/pii/similarity")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # 定义配对的数字组合 (forgetXX 和 retainYY 的 XX + YY = 100)
    split = [
        # 'scal-5',
        # 'scal-10',
        # 'scal-20',
    ...
    for scal_path in split:
        # 构建文件路径
        data_path = os.path.join(base_path, f"{scal_path}/raw/synthetic_data.json")
        
        print(f"\nProcessing split: {scal_path}")
        
        # 检查文件是否存在
        if not (os.path.exists(data_path)):
            print(f"Warning: Missing {data_path} files, skipping")
            continue
        
        # 计算相似度矩阵
        _, _, similarity_matrix = load_and_compute_similarity(
            data_path, model, tokenizer
        )
        
        print(f"Cosine similarity matrix shape: {similarity_matrix.shape}")
        # 保存结果
        output_filename = f"similarity-{scal_path}.pt"
...
if __name__ == "__main__":
    ...
    if corpus == 'pii':
        base_path = "/data/home/jvnting/cnpo/data/pii"
    process_paired_files(base_path, model, tokenizer)
