...
from metrics.verbmem_pii import eval as eval_verbmem_pii
from metrics.privleak import eval as eval_privleak
from metrics.knowmem import eval as eval_knowmem
from metrics.knowmem_pii import eval as eval_knowmem_pii
...
def eval_model(
    ...
    if 'knowmem_f' in metrics:
        if corpus == 'pii':
            forget_qa = read_json(knowmem_forget_qa_file)
            icl_forget_qa = read_json(knowmem_forget_qa_icl_file)
            agg, log = eval_knowmem_pii(
                qa=forget_qa,
                icl_qa=icl_forget_qa,
                model=model, tokenizer=tokenizer,
    ...
    if 'knowmem_r' in metrics:
        if corpus == 'pii':
            retain_qa = read_json(knowmem_retain_qa_file)
            icl_retain_qa = read_json(knowmem_retain_qa_icl_file)
            agg, log = eval_knowmem_pii(
                qa=retain_qa,
                icl_qa=icl_retain_qa,
                model=model, tokenizer=tokenizer,
                max_new_tokens=knowmem_max_new_tokens
            )
        else:
...
def load_then_eval_models(
    model_dirs: List[str],
    basemodel_dir: List[str],
    peft: bool,
    names: List[str],
    corpus: Literal['news', 'books', 'pii'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--basemodel_dir', type=str, default=[])
    ...
    parser.add_argument('--metrics', type=str, nargs='+', default=PII_METRICS)
    parser.add_argument('--scal', type=int, default=5)
