...
def finetune(
    ...
    training_args = transformers.TrainingArguments(
        ...
        gradient_accumulation_steps=16,  # 累积4个batch的梯度再更新
        dataloader_num_workers=4,      # 数据加载线程数
        # dataloader_num_workers=4,               # 建议: GPU数量 × 2
        # gradient_accumulation_steps=2,          # 总batch_size = per_device_batch_size × GPU数 × 该值
        max_grad_norm=1.0,  # 梯度裁剪
        # optim_args="fused=True",  # 启用融合优化器
    )
